---
title: "06_Multiple_lineal_regresion_apertura+vol"
...
data <- read.csv('./dataset/IBEX35/final_historic_data_IBEX35_toR.csv', sep = ',', header = T, fileEncoding = 'utf-8')
glimpse(data)
```
...
- Las variables que tienen una mayor relación lineal con ultimo son: apertura (r=0.99470499), maximo (r=0.99695352) y minimo (r=0.99809732)
...
```{r}
bss.model <- regsubsets(ultimo ~ apertura + vol, data = data_training)
summary(bss.model)
```
...
$$
ultimo = 8.687723 + 1.000061 \cdot apertura - 0.068307  \cdot vol
$$
El modelo explica que a mayor valor de ultimo se produce un incremento en el valor de apertura y una disminución en el volúmen de operaciones de compraventa.
...
```
El coeficiente más importante es la apertura, así que lo que más influye en el valor de cierre es el valor de apertura. El volúmen de las operaciones de compraventa es lo menos importante.
...
- Segundo gráfico: tipo Normal Q-Q. Compara los residuos estandarizados con la distribución teorica Normal. Si los puntos se alejan de la linea normal, indica que los residuos no se ajustan a una dsitribución Normal.
...
```
Este valor "7960.96" es algo peor que el obtenido en los datos de train "7363.133".
...
for(fold in fold_index_list){  
  ...
  test_data <- data[fold, ]
  mod_1 <- lm(ultimo ~ apertura, data = training_data)
  mod_2 <- lm(ultimo ~ apertura + vol, data = training_data)
...
# Entre
kfold_training_model <- train(ultimo ~ apertura + vol, data = data_training, method = "lm",
