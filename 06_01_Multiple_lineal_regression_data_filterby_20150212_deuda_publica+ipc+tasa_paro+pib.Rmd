...
En esta primera prueba vamos a utilizar el dataset con los datos desde el 12-02-2015. Se filtra por esta fecha porque viene marcado por el dataset de AENA que sólo tiene datos desde esa fecha.
...
data <- read.csv('./dataset/IBEX35/final_historic_data_IBEX35_with_metrics_filter_by_20150212_toR.csv', sep = ',', header = T, fileEncoding = 'utf-8')
```
...
- Las variables que tienen una mayor relación lineal con ultimo son: apertura (r=00.99629991), maximo (r=0.99792433) y minimo (r=0.99860483)
...
- Las variables que presenta bastante relación lineal con ultimo son: deuda_publica (r=-0.53327978), ipc (r=0.26749785), tasa_paro (r=0.38616729) y en menor medida pib (r=0.56109609)
...
```
Por el criterio "del codo" en todos los estadísticos tiene sentido tomar 2 variables.
...
$$
ultimo = 2.685e\+04 -1.433e\-02 \cdot deuda_publica + 3.183e\+02  \cdot ipc -7.491e\+01  \cdot tasa_paro + 7.812e\+01  \cdot pib
$$
El modelo explica que a mayor valor de ultimo se produce un decremento en la deuda pública y la tasa de paro.
...
```
Los coeficientes más importante son la deuda_publica, ipc y pib, así que son los que más influyen en el valor de cierre. La tasa de paro es lo menos importante.
...
- Segundo gráfico: tipo Normal Q-Q. Compara los residuos estandarizados con la distribución teórica Normal. Si los puntos se alejan de la linea normal, indica que los residuos no se ajustan a una distribución Normal.
...
```{r, fig.align='center'}
saveRDS(lm_fit_ultimo_all, "./models/lm_fit_ultimo_filter_data_deuda_publica+ipc+tasa_paro+pib.rds")
```

Cargamos el modelo guardado para usarlo con los modelos de test:
```{r, fig.align='center'}
save_lm_fit_ultimo_all <- readRDS("./models/lm_fit_ultimo_filter_data_deuda_publica+ipc+tasa_paro+pib.rds")
...
```
Este valor "512492.6" es algo mejor que el obtenido en los datos de train "556802.8".
